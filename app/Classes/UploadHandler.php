...
class UploadHandler
{
	...
	protected function imagick_create_scaled_image( $file_name, $version, $options )
	{
		...
		$success = $image->resizeImage( $new_width, $new_height, isset( $options['filter'] ) ? $options['filter'] : \imagick::FILTER_LANCZOS, isset( $options['blur'] ) ? $options['blur'] : 1, $new_width && $new_height ); // fit image into constraints
		                                                                                                                                                                                                                     // if
		                                                                                                                                                                                                                     // not to be cropped
	...
	{
		$file = new \stdClass();
		$file->name = str_replace( ' ', '-', $this->get_file_name( $uploaded_file, $name, $size, $type, $error, $index, $content_range ) );
	...
	protected function basename( $filepath, $suffix = null )
	{
		...
		return strtolower( substr( basename( 'X' . $splited[count( $splited ) - 1], $suffix ), 1 ) );
	}
}
