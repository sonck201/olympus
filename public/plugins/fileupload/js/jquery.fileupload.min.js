...
e.type(a.paramName)?a.paramName[0]:a.paramName;a.headers=e.extend({},a.headers);a.contentRange&&(a.headers["Content-Range"]=a.contentRange);f&&!a.blob&&this._isInstanceOf("File",d)||(a.headers["Content-Disposition"]='attachment; filename="'+encodeURI(d.name)+'"');f?e.support.xhrFormDataFileUpload&&(a.postMessage?(c=this._getFormData(a),a.blob?c.push({name:g,value:a.blob}):e.each(a.files,function(b,d){c.push({name:"array"===e.type(a.paramName)&&a.paramName[b]||g,value:d})})):(b._isInstanceOf("FormData",
...
"pending"===c._getDeferredState(this._processQueue)};b.progress=function(){return this._progress};b.response=function(){return this._response}},_getUploadedBytes:function(a){return(a=(a=(a=a.getResponseHeader("Range"))&&a.split("-"))&&1<a.length&&parseInt(a[1],10))&&a+1},_chunkedUpload:function(a,b){a.uploadedBytes=a.uploadedBytes||0;var c=this,d=a.files[0],f=d.size,g=a.uploadedBytes,k=a.maxChunkSize||f,h=this._blobSlice,l=e.Deferred(),p=l.promise(),m,n;if(!(this._isXHRUpload(a)&&h&&(g||k<f))||a.data)return!1;
if(b)return!0;if(g>=f)return d.error=a.i18n("uploadedBytes"),this._getXHRPromise(!1,a.context,[null,"error",d.error]);n=function(){var b=e.extend({},a),p=b._progress.loaded;b.blob=h.call(d,g,g+k,d.type);b.chunkSize=b.blob.size;b.contentRange="bytes "+g+"-"+(g+b.chunkSize-1)+"/"+f;c._initXHRData(b);c._initProgressListener(b);m=(!1!==c._trigger("chunksend",null,b)&&e.ajax(b)||c._getXHRPromise(!1,b.context)).done(function(d,h,k){g=c._getUploadedBytes(k)||g+b.chunkSize;p+b.chunkSize-b._progress.loaded&&
c._onProgress(e.Event("progress",{lengthComputable:!0,loaded:g-b.uploadedBytes,total:g-b.uploadedBytes}),b);a.uploadedBytes=b.uploadedBytes=g;b.result=d;b.textStatus=h;b.jqXHR=k;c._trigger("chunkdone",null,b);c._trigger("chunkalways",null,b);g<f?n():l.resolveWith(b.context,[d,h,k])}).fail(function(a,d,e){b.jqXHR=a;b.textStatus=d;b.errorThrown=e;c._trigger("chunkfail",null,b);c._trigger("chunkalways",null,b);l.rejectWith(b.context,[a,d,e])})};this._enhancePromise(p);p.abort=function(){return m.abort()};
...
null,d)},_onSend:function(a,b){b.submit||this._addConvenienceMethods(a,b);var c=this,d,f,g,k,h=c._getAJAXSettings(b),l=function(){c._sending+=1;h._bitrateTimer=new c._BitrateTimer;return d=d||((f||!1===c._trigger("send",e.Event("send",{delegatedEvent:a}),h))&&c._getXHRPromise(!1,h.context,f)||c._chunkedUpload(h)||e.ajax(h)).done(function(a,b,d){c._onDone(a,b,d,h)}).fail(function(a,b,d){c._onFail(a,b,d,h)}).always(function(a,b,d){c._onAlways(a,b,d,h);--c._sending;--c._active;if(h.limitConcurrentUploads&&
h.limitConcurrentUploads>c._sending)for(a=c._slots.shift();a;){if("pending"===c._getDeferredState(a)){a.resolve();break}a=c._slots.shift()}0===c._active&&c._trigger("stop")})};this._beforeSend(a,h);return this.options.sequentialUploads||this.options.limitConcurrentUploads&&this.options.limitConcurrentUploads<=this._sending?(1<this.options.limitConcurrentUploads?(g=e.Deferred(),this._slots.push(g),k=g.then(l)):k=this._sequence=this._sequence.then(l,l),k.abort=function(){f=[void 0,"abort","abort"];
return d?d.abort():(g&&g.rejectWith(h.context,f),l())},this._enhancePromise(k)):l()},_onAdd:function(a,b){var c=this,d=!0,f=e.extend({},this.options,b),g=b.files,k=g.length,h=f.limitMultiFileUploads,l=f.limitMultiFileUploadSize,p=f.limitMultiFileUploadSizeOverhead,m=0,n=this._getParamName(f),q,r,t=0;if(!k)return!1;l&&void 0===g[0].size&&(l=void 0);if((f.singleFileUploads||h||l)&&this._isXHRUpload(f))if(f.singleFileUploads||l||!h)if(!f.singleFileUploads&&l)for(r=[],q=[],f=0;f<k;f+=1){if(m+=g[f].size+
p,f+1===k||m+g[f+1].size+p>l||h&&f+1-t>=h)r.push(g.slice(t,f+1)),m=n.slice(t,f+1),m.length||(m=n),q.push(m),t=f+1,m=0}else q=n;else for(r=[],q=[],f=0;f<k;f+=h)r.push(g.slice(f,f+h)),m=n.slice(f,f+h),m.length||(m=n),q.push(m);else r=[g],q=[n];b.originalFiles=g;e.each(r||g,function(f,g){var h=e.extend({},b);h.files=r?g:[g];h.paramName=q[f];c._initResponseObject(h);c._initProgressObject(h);c._addConvenienceMethods(a,h);return d=c._trigger("add",e.Event("add",{delegatedEvent:a}),h)});return d},_replaceFileInput:function(a){var b=
a.fileInput,c=b.clone(!0),d=b.is(document.activeElement);a.fileInputClone=c;e("<form></form>").append(c)[0].reset();b.after(c).detach();d&&c.focus();e.cleanData(b.unbind("remove"));this.options.fileInput=this.options.fileInput.map(function(a,d){return d===b[0]?c[0]:d});b[0]===this.element[0]&&(this.element=c)},_handleFileTreeEntry:function(a,b){var c=this,d=e.Deferred(),f=[],g,k=function(b){b&&!b.entry&&(b.entry=a);d.resolve([b])},h=function(e){c._handleFileTreeEntries(e,b+a.name+"/").done(function(a){d.resolve(a)}).fail(k)},
l=function(){g.readEntries(function(a){a.length?(f=f.concat(a),l()):h(f)},k)};b=b||"";a.isFile?a._file?(a._file.relativePath=b,d.resolve(a._file)):a.file(function(a){a.relativePath=b;d.resolve(a)},k):a.isDirectory?(g=a.createReader(),l()):d.resolve([]);return d.promise()},_handleFileTreeEntries:function(a,b){var c=this;return e.when.apply(e,e.map(a,function(a){return c._handleFileTreeEntry(a,b)})).then(function(){return Array.prototype.concat.apply([],arguments)})},_getDroppedFiles:function(a){a=
...
return new RegExp(a.join("/"),b)},_isRegExpOption:function(a,b){return"url"!==a&&"string"===e.type(b)&&/^\/.*\/[igm]{0,3}$/.test(b)},_initDataAttributes:function(){var a=this,b=this.options,c=this.element.data();e.each(this.element[0].attributes,function(d,e){var f=e.name.toLowerCase(),k;/^data-/.test(f)&&(f=f.slice(5).replace(/-[a-z]/g,function(a){return a.charAt(1).toUpperCase()}),k=c[f],a._isRegExpOption(f,k)&&(k=a._getRegExp(k)),b[f]=k)})},_create:function(){this._initDataAttributes();this._initSpecialOptions();
